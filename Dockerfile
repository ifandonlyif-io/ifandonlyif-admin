...
RUN wget https://github.com/oven-sh/bun/releases/download/bun-v0.7.3/SHASUMS256.txt &&\
...
COPY . .
# RUN NODE_OPTIONS=--max_old_space_size=650 ./bun-linux-aarch64/bun --smol run build
RUN NODE_OPTIONS=--max_old_space_size=650 npm run build
